...
# Part 5.4A – Interview Questions & Answers (1–15)

This section contains the most frequently asked **Linux Services** interview questions. These questions are commonly asked in **Linux Administration, Technical Support, DevOps, AWS, Cloud Computing, and System Administration** interviews.

---

# Q1. What are Services in Linux?

## Professional Answer

A **Service** is a background process that runs continuously or starts automatically to perform specific tasks without requiring direct user interaction.

Services usually start during system boot and remain active until they are stopped or the system shuts down.
...
# Q2. What is a Daemon in Linux?

## Professional Answer

A **Daemon** is a background process that waits for requests and performs tasks automatically.

Most Linux services run as daemon processes. Their names usually end with the letter **d**.
...
**systemd** is the modern initialization system (Init System) and service manager used in most Linux distributions.
...
| Process | Service |
|---------|----------|
| Running program | Background program managed by the system |
| Starts manually or automatically | Usually starts automatically |
| Can terminate after completing a task | Usually keeps running continuously |
| Example: `vim` | Example: `sshd` |
...
# Q6. How do you check the status of a service?
...
# Q9. What is the difference between Restart and Reload?

## Professional Answer

| Restart | Reload |
|----------|---------|
| Stops and starts the service again | Reloads configuration without stopping the service |
| May interrupt active connections | Usually does not interrupt active connections |
...
# Q10. What is the purpose of `systemctl enable`?

## Professional Answer

The `enable` command configures a service to start automatically whenever the system boots.
...
# Q11. What is the purpose of `systemctl disable`?

## Professional Answer

The `disable` command prevents a service from starting automatically during system boot.
...
# Q12. Why doesn't `systemctl` work in WSL?

## Professional Answer

By default, many WSL installations do not use **systemd** as PID 1 (the init process). Since `systemctl` communicates with `systemd`, it cannot manage services unless `systemd` has been enabled.

Instead, WSL users commonly use:
...
This is a common interview question for candidates using WSL.

---

# Q13. How do you check if a service is running?
...
# Q14. How do you view service logs?

## Professional Answer

Service logs help troubleshoot failures and monitor service activity.
...
If users cannot log in through SSH, administrators review the SSH logs to identify authentication failures or configuration issues.

---

# Q15. Why are Linux Services important?

## Professional Answer

Linux services are essential because they keep critical applications and system components running automatically.

They provide:

- Remote access (SSH)
- Website hosting (Nginx/Apache)
- Database services (MySQL)
- Container management (Docker)
- Scheduled automation (Cron)
- Network management

Without services, most server applications would need to be started manually after every reboot.

### Real-World Example

When an AWS EC2 instance starts:
...
This ensures the server is ready to serve users without manual intervention.
...
|----------|--------------|
| What is a Service? | A background process that performs tasks automatically. |
| What is a Daemon? | A background process that usually ends with `d`. |
| What is systemd? | The modern Linux init system and service manager. |
| What is systemctl? | Command used to manage systemd services. |
| Process vs Service | Process is any running program; service is a managed background process. |
| Check service status | `systemctl status` or `service status` |
| Start service | `systemctl start` |
| Stop service | `systemctl stop` |
| Restart vs Reload | Restart stops and starts; reload applies config without full restart. |
| Enable service | Starts automatically at boot. |
| Disable service | Prevents automatic startup. |
| Why doesn't systemctl work in WSL? | Because systemd may not be running as PID 1. |
